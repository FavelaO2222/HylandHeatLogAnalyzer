...
from database.research_records import require_row, resolve_subject_entity
...
class ResearchRecordsTests(unittest.TestCase):
    ...
    def test_resolve_subject_entity_matches_by_name_or_canonical_name(self):
        entity_id = self.entity(name='OfficerLee')
        self.assertEqual(resolve_subject_entity(self.connection, 'officerlee'), entity_id)
        self.assertEqual(resolve_subject_entity(self.connection, 'OfficerLee'), entity_id)

    def test_resolve_subject_entity_unmatched_raises(self):
        with self.assertRaisesRegex(ValueError, "No entity named 'Nobody'"):
            resolve_subject_entity(self.connection, 'Nobody')

    def test_resolve_subject_entity_ambiguous_raises_with_disambiguation_hint(self):
        ...
        with self.assertRaisesRegex(ValueError, 'matches more than one entity.*subject_entity_id to disambiguate'):
            resolve_subject_entity(self.connection, 'OfficerLee')
